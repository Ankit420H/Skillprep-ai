...
function Interview({ params }) {
  ...
  const fetchInterview = async () => {
    try {
      const res = await fetch(`/api/get-interview?mockId=${params.interviewId}`);
      if (res.ok) setData(await res.json());
      else toast.error("Error fetching interview details");
    } catch (error) {
          ...
          <Card className="border-border shadow-soft">
            ...
            <CardContent className="space-y-6 pt-6">
              <div>
                ...
                <div className="text-lg font-medium text-foreground mt-1">{data.jobPosition}</div>
              </div>
              <div>
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Tech Stack</label>
                <div className="text-base text-foreground mt-1 leading-relaxed">{data.jobDesc}</div>
              </div>
              <div>
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Experience Required</label>
                <div className="text-lg font-medium text-foreground mt-1">{data.jobExperience} Years</div>
              </div>
            </CardContent>
          </Card>
