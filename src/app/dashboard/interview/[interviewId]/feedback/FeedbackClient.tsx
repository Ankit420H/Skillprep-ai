...
interface Props {
  feedbackList: UserAnswerType[];
}

const FeedbackClient = ({ feedbackList }: Props) => {
  const router = useRouter();

  const validRatings = feedbackList.map((item) => parseFloat(item.rating || "0")).filter((r) => !isNaN(r));
  const total = validRatings.reduce((sum, r) => sum + r, 0);
  const avg = validRatings.length > 0 ? (total / validRatings.length).toFixed(1) : null;
                ...
                <CollapsibleTrigger className="w-full">
                  <div className="flex items-center justify-between p-4 bg-gray-100 hover:bg-gray-200 transition-colors">
                    <div className="flex items-center gap-3">
                      <Target
                        className={`h-5 w-5 ${parseFloat(item.rating || "0") >= 7
                          ? "text-green-500"
